...
type PkgBag struct {
	...
	func newPkgBag (schema *Schema) (bag *PkgBag) {
		...
		for i := 0; newImpname; i++ {
			newImpname = false
			for _, s := range schema.allSchemas() { for ns, _ := range s.XMLNamespaces { if ns == bag.impName { newImpname = true; break } } }
			if newImpname { bag.impName = sfmt("xsdt", i) }
		}
		bag.imports, bag.impsUsed, bag.lines = map[string]string {}, map[string]bool {}, []string { "//\tAuto-generated by the \"go-xsd\" package located at:", "//\t\tgithub.com/metaleap/go-xsd", "//\tComments on types and fields (if any) are from the XSD file located at:", "//\t\t" + bag.Schema.loadUri, "package gopkg_" + bag.safeName(bag.Schema.RootSchema().loadUri), "" }
	...
	func (me *PkgBag) addType (elem element, n, t string, a ... *Annotation) (dt *declType) {
		...
		if elem != nil { me.declElemTypes[elem] = append(me.declElemTypes[elem], dt) }
		return
	}
	...
	func (me *PkgBag) AnonName (n string) (an xsdt.NCName) {
		...
		n = "Txsd" + n
		an = xsdt.NCName(n)
		if c = me.anonCounts[n]; c > 0 { an += xsdt.NCName(fmt.Sprintf("%v", c)) }
		me.anonCounts[n] = c + 1
		return
	}
	...
	func (me *PkgBag) assembleSource () string {
		...
		if len(me.allNotations) > 0 {
			...
			for _, not := range me.allNotations { not.makePkg(me) }
			me.appendFmt(true, "}")
		}
		for _, att := range me.allAtts { render(att) }
	...
	func (me *PkgBag) checkType (typeSpec string) {
		...
		if dt = me.declTypes[tn]; (dt != nil) && (len(dt.EquivalentTo) > 0) { tn = dt.EquivalentTo; dt = me.declTypes[tn] }
		if pos := strings.Index(tn, "."); pos > 0 { me.impsUsed[tn[: pos]] = true }
		if dt != nil { dt.render(me) } // else if (tn != "string") && (tn != "bool") && (len(tn) > 0) && !strings.Contains(tn, ".") { println("TYPE NOT FOUND: " + tn) }
	}

	func (me *PkgBag) isParseType (typeRef string) (bool) {
		for pt, _ := range me.parseTypes { if typeRef == pt { return true } }
		return false
	}
...
type declMethod struct {
	...
	func (me *declMethod) render (bag *PkgBag, dt *declType) {
		...
		bag.appendFmt(true, "func (me %s) %s %s { %s }", me.ReceiverType, util.Ifs(strings.Contains(me.Name, "("), me.Name, me.Name + " ()"), rt, strings.Replace(me.Body, me.ReturnType, rt, -1))
	}
...
type declType struct {
	...
	func (me *declType) checkForEquivalents (bag *PkgBag) {
		if (len(me.EquivalentTo) == 0) && (strings.HasPrefix(me.Name, "Txsd") || strings.HasPrefix(me.Name, idPrefix)) {
			for _, dt := range bag.declWrittenTypes {
				if (dt != me) && (len(dt.EquivalentTo) == 0) && me.equivalentTo(dt) { me.EquivalentTo = dt.Name }
			}
		}
	}
	...
	func (me *declType) equivalentTo (dt *declType) bool {
		...
		for e, _ := range dt.Embeds { sdt = append(sdt, e) }
		if !ustr.Equivalent(sme, sdt) { return false }
		sme, sdt = []string {}, []string {}
		for _, f := range me.Fields { sme = append(sme, f.Name + f.Type + f.XmlTag) }
	...
	func (me *declType) render (bag *PkgBag) {
		if !me.rendered {
			...
			if me.checkForEquivalents(bag); len(me.EquivalentTo) == 0 {
				for _, ann := range me.Annotations { if ann != nil { ann.makePkg(bag) } }
				for _, e := range me.Embeds { bag.checkType(e.Name) }
				for _, f := range me.Fields { bag.checkType(f.Type) }
				for _, m := range me.Methods { bag.checkType(m.ReturnType) }
				if len(me.Type) > 0 {
					bag.checkType(me.Type)
					bag.appendFmt(true, "type %s %s", me.Name, me.Type)
				} else {
					bag.appendFmt(false, "type %s struct {", me.Name)
