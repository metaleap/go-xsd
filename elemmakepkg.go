...
func (me *Attribute) makePkg (bag *PkgBag) {
	...
	if len(me.Ref) > 0 {
		...
		if bag.attRefImps[me], bag.attsKeys[me] = impName, key; len(bag.attsCache[key]) == 0 { bag.attsCache[key] = tmp }
	} else {
		...
		if len(bag.attsCache[key]) == 0 {
			...
			td.addField(me, safeName, typeName, util.Ifs(len(bag.Schema.TargetNamespace) > 0, bag.Schema.TargetNamespace.String() + " ", "") + me.Name.String() + ",attr")
...
func (me *ComplexType) makePkg (bag *PkgBag) {
	...
	for elGr, _ = range allElemGroups { subMakeElemGroup(bag, td, elGr, grsDone, anns(nil, me.ComplexContent) ...) }
	for el, _ = range allElems { subMakeElem(bag, td, el, elsDone, 1, anns(me.All, nil) ...) }
	for _, ch := range allChoices {
		for _, el = range ch.Elements { subMakeElem(bag, td, el, elsDone, ch.hasAttrMaxOccurs.Value(), ch.Annotation) }
		for _, elGr = range ch.Groups { subMakeElemGroup(bag, td, elGr, grsDone, ch.Annotation) }
	}
	for _, seq := range allSeqs {
		for _, el = range seq.Elements { subMakeElem(bag, td, el, elsDone, seq.hasAttrMaxOccurs.Value(), seq.Annotation) }
		for _, elGr = range seq.Groups { subMakeElemGroup(bag, td, elGr, grsDone, seq.Annotation) }
	}
...
func (me *Element) makePkg (bag *PkgBag) {
	...
	if len(me.Ref) > 0 {
		...
		for pref, cache := range map[string]map[string]string { "HasElem_": bag.elemsCacheOnce, "HasElems_": bag.elemsCacheMult } {
			...
			if bag.elemRefImps[me], bag.elemKeys[me] = impName, key; len(cache[key]) == 0 { cache[key] = tmp }
		}
	} else {
		safeName = bag.safeName(me.Name.String())
		if typeName = me.Type.String(); (len(typeName) == 0) && ((me.ComplexType != nil) || (len(me.SimpleTypes) > 0)) {
			...
			if len(typeName) == 0 { typeName = bag.xsdStringTypeRef() }
			if typeName = bag.resolveQnameRef(typeName, "T", &impName); bag.Schema.RootSchema().globalComplexType(bag, typeName) != nil { asterisk = "*" }
		}
		if defVal = me.Default; len(defVal) == 0 { defName, defVal = "Fixed", me.Fixed }
		...
		if valueType = bag.simpleContentValueTypes[typeName]; len(valueType) == 0 { valueType = typeName }
		for pref, cache := range map[string]map[string]string { "HasElem_": bag.elemsCacheOnce, "HasElems_": bag.elemsCacheMult } {
...
func (me *Group) makePkg (bag *PkgBag) {
	...
	if len(me.Ref) > 0 {
		...
		if me.All != nil { for _, el = range me.All.Elements { subMakeElem(bag, td, el, elsDone, 1, me.All.Annotation) } }
		for _, ch := range choices {
			for _, el = range ch.Elements { subMakeElem(bag, td, el, elsDone, ch.hasAttrMaxOccurs.Value(), ch.Annotation) }
			for _, gr = range ch.Groups { subMakeElemGroup(bag, td, gr, grsDone, ch.Annotation) }
		}
		for _, seq := range seqs {
			for _, el = range seq.Elements { subMakeElem(bag, td, el, elsDone, seq.hasAttrMaxOccurs.Value(), seq.Annotation) }
			for _, gr = range seq.Groups { subMakeElemGroup(bag, td, gr, grsDone, seq.Annotation) }
		}
	}
...
func (me *List) makePkg (bag *PkgBag) {
	...
	me.elemBase.beforeMakePkg(bag)
	me.hasElemsSimpleType.makePkg(bag)
	if len(rtr) == 0 { rtr = me.SimpleTypes[0].Name.String() }
	safeName = bag.safeName(ustr.PrependIf(bag.Stacks.CurSimpleType().Name.String(), "T"))
	var doc = sfmt("%v defines a String containing a whitespace-separated list of %v values. This Values() method creates and returns a slice of all elements in that list.", safeName, rtr)
	var body string
...
func (me *SimpleType) makePkg (bag *PkgBag) {
	...
	if isPt = bag.isParseType(baseType); isPt { bag.parseTypes[safeName] = true }
...
func anns (a *All, cc *ComplexContent) (anns []*Annotation) {
	if (a != nil) && (a.Annotation != nil) { anns = append(anns, a.Annotation) }
	if (cc != nil) {
		if (cc.Annotation != nil) { anns = append(anns, cc.Annotation) }
